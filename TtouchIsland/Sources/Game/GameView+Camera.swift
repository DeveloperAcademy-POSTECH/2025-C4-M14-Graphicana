...
extension GameView {
    ...
    func setupWorldCamera(target: Entity) -> Entity {  // target: 카메라가 바라볼 목표 entity
        ...
        let worldCamera = Entity(
            components: worldCameraComponent,
    ...
    func cameraZoomInOut(delta: Float) {
        // gameCamera가 존재하고 WorldCameraComponent가 붙어 있는지 확인
        guard let gameCamera = manager.gameCamera,
            var camera = gameCamera.components[WorldCameraComponent.self]
        else { return }

        // radius 값을 delta로 나누어 newRadius를 계산합니다
        // delta > 1이면 줌 인, delta < 1이면 줌 아웃
        // camera.radius / delta 값을 0.3 ~ 10.0 범위로 제한
        let newRadius = min(max(camera.radius / delta, 0.3), 10.0)
        camera.radius = newRadius
        // 변경된 카메라를 엔티티에 적용
        gameCamera.components.set(camera)
    }
}
